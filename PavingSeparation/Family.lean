...
noncomputable def skewMatrix : (m : ℕ) → Matrix (Cube m) (Cube m) ℂ
  | 0 => 0
  | m + 1 => fromBlocks (skewMatrix m) (skewMatrix m + 1) (skewMatrix m - 1) (-skewMatrix m)

@[simp] theorem skew_zero : skewMatrix 0 = 0 := rfl

theorem skew_succ (m : ℕ) :
    skewMatrix (m + 1) =
      fromBlocks (skewMatrix m) (skewMatrix m + 1) (skewMatrix m - 1) (-skewMatrix m) := rfl

@[simp] theorem skew_inl_inl (m : ℕ) (i j : Cube m) :
    skewMatrix (m + 1) (Sum.inl i) (Sum.inl j) = skewMatrix m i j := rfl

@[simp] theorem skew_inr_inr (m : ℕ) (i j : Cube m) :
    skewMatrix (m + 1) (Sum.inr i) (Sum.inr j) = -skewMatrix m i j := rfl

@[simp] theorem skew_inl_inr (m : ℕ) (i j : Cube m) :
    skewMatrix (m + 1) (Sum.inl i) (Sum.inr j) = (skewMatrix m + 1) i j := rfl

@[simp] theorem skew_inr_inl (m : ℕ) (i j : Cube m) :
    skewMatrix (m + 1) (Sum.inr i) (Sum.inl j) = (skewMatrix m - 1) i j := rfl

@[simp] theorem skew_diag (m : ℕ) (i : Cube m) : skewMatrix m i i = 0 := by
  induction m with
  | zero => rfl
  | succ m ih => cases i <;> simp [ih]

theorem skew_offdiag_eq_one_or_neg_one (m : ℕ) {i j : Cube m} (hij : i ≠ j) :
    skewMatrix m i j = 1 ∨ skewMatrix m i j = -1 := by
  induction m with
...
@[simp] theorem skew_offdiag_norm (m : ℕ) {i j : Cube m} (hij : i ≠ j) :
    ‖skewMatrix m i j‖ = 1 := by
  rcases skew_offdiag_eq_one_or_neg_one m hij with h | h <;> simp [h]

@[simp] theorem skew_conj_apply (m : ℕ) (i j : Cube m) :
    star (skewMatrix m i j) = skewMatrix m i j := by
  by_cases hij : i = j
  · subst j; simp
  · rcases skew_offdiag_eq_one_or_neg_one m hij with h | h <;> simp [h]

theorem skew_conjTranspose (m : ℕ) : (skewMatrix m)ᴴ = -skewMatrix m := by
  induction m with
  | zero => simp
  | succ m ih =>
...
theorem skew_transpose (m : ℕ) : (skewMatrix m)ᵀ = -skewMatrix m := by
  ext i j
...
theorem skew_mul_self (m : ℕ) :
    skewMatrix m * skewMatrix m = (-((2 : ℂ) ^ m - 1)) • (1 : Matrix (Cube m) (Cube m) ℂ) := by
  induction m with
  | zero => simp
  | succ m ih =>
      rw [skew_succ, fromBlocks_multiply]
      have hdiag : skewMatrix m * skewMatrix m + (skewMatrix m + 1) * (skewMatrix m - 1) =
          (-((2 : ℂ) ^ (m + 1) - 1)) • (1 : Matrix (Cube m) (Cube m) ℂ) := by
        calc
          _ = skewMatrix m * skewMatrix m + skewMatrix m * skewMatrix m - 1 := by noncomm_ring
          _ = _ := by
            rw [ih]
            ext i j
            ...
            rw [pow_succ]
            ring
      have hoff : skewMatrix m * (skewMatrix m + 1) + (skewMatrix m + 1) * -skewMatrix m = 0 := by
        noncomm_ring
      have hother : (skewMatrix m - 1) * skewMatrix m + -skewMatrix m * (skewMatrix m - 1) = 0 := by
        noncomm_ring
      have hdiag' : (skewMatrix m - 1) * (skewMatrix m + 1) + -skewMatrix m * -skewMatrix m =
          (-((2 : ℂ) ^ (m + 1) - 1)) • (1 : Matrix (Cube m) (Cube m) ℂ) := by
...
theorem skew_norm (m : ℕ) : ‖skewMatrix m‖ = Real.sqrt ((2 : ℝ) ^ m - 1) := by
  have hstar : (skewMatrix m)ᴴ * skewMatrix m =
      (((2 : ℝ) ^ m - 1 : ℝ) : ℂ) • (1 : Matrix (Cube m) (Cube m) ℂ) := by
    rw [skew_conjTranspose, neg_mul, skew_mul_self, ← neg_smul, neg_neg]
    congr 1
    push_cast
    rfl
  have hn := Matrix.l2_opNorm_conjTranspose_mul_self (skewMatrix m)
...
noncomputable def pavingMatrix (m : ℕ) : Matrix (Cube m) (Cube m) ℂ :=
  (Complex.I / (Real.sqrt ((2 : ℝ) ^ m - 1) : ℂ)) • skewMatrix m

@[simp] theorem family_zero : pavingMatrix 0 = 0 := by simp [pavingMatrix]

@[simp] theorem family_diag (m : ℕ) (i : Cube m) : pavingMatrix m i i = 0 := by
  simp [pavingMatrix]

@[simp] theorem family_trace (m : ℕ) : Matrix.trace (pavingMatrix m) = 0 := by
  simp [Matrix.trace]

theorem family_isHermitian (m : ℕ) : (pavingMatrix m).IsHermitian := by
  change (pavingMatrix m)ᴴ = pavingMatrix m
  rw [pavingMatrix, conjTranspose_smul, skew_conjTranspose]
  simp only [star_div₀, Complex.star_def, Complex.conj_I, Complex.conj_ofReal, neg_div,
    neg_smul_neg]

theorem family_mul_self {m : ℕ} (hm : 0 < m) :
    pavingMatrix m * pavingMatrix m = (1 : Matrix (Cube m) (Cube m) ℂ) := by
  ...
  have hsq : (Real.sqrt ((2 : ℝ) ^ m - 1) : ℂ) ^ 2 = (2 : ℂ) ^ m - 1 := by
    ...
    exact_mod_cast h
  rw [pavingMatrix, smul_mul_smul, skew_mul_self, smul_smul]
  have hcoef :
...
theorem family_mem_unitaryGroup {m : ℕ} (hm : 0 < m) :
    pavingMatrix m ∈ Matrix.unitaryGroup (Cube m) ℂ := by
  rw [Matrix.mem_unitaryGroup_iff, Matrix.star_eq_conjTranspose,
    (family_isHermitian m).eq]
  exact family_mul_self hm

theorem family_norm {m : ℕ} (hm : 0 < m) : ‖pavingMatrix m‖ = 1 := by
  have hr := Real.sqrt_pos.2 (order_sub_one_pos hm)
  rw [pavingMatrix, norm_smul, norm_div, Complex.norm_I, Complex.norm_real,
    Real.norm_of_nonneg hr.le, skew_norm]
  exact one_div_mul_cancel hr.ne'

theorem family_offdiag_norm (m : ℕ) {i j : Cube m} (hij : i ≠ j) :
    ‖pavingMatrix m i j‖ = 1 / Real.sqrt ((2 : ℝ) ^ m - 1) := by
  simp only [pavingMatrix, Matrix.smul_apply, smul_eq_mul, norm_mul, norm_div, Complex.norm_I,
...
theorem family_offdiag_norm_sq (m : ℕ) {i j : Cube m} (hij : i ≠ j) :
    ‖pavingMatrix m i j‖ ^ 2 = 1 / ((2 : ℝ) ^ m - 1) := by
...
theorem family_offdiag_normSq (m : ℕ) {i j : Cube m} (hij : i ≠ j) :
    Complex.normSq (pavingMatrix m i j) = 1 / ((2 : ℝ) ^ m - 1) := by
